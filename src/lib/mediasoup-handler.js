...
export class MediasoupHandler {
  ...
  async join() { this.socket = io(serverUrl); this.setupSocketEvents(); }
  close() { this.socket?.disconnect(); this.localMediaStream?.getTracks().forEach(t => t.stop()); this.sendTransport?.close(); this.recvTransport?.close(); } // CHANGED: Also close recvTransport
  ...
  setupSocketEvents() {
    ...
    this.socket.on('consumer-closed', ({ consumerId }) => {
      const consumer = this.consumers.get(consumerId); if (!consumer) return;
      consumer.close(); this.consumers.delete(consumerId);
      // Note: You might want to update the participant's stream to null instead of removing them entirely
      // if they only closed one stream (e.g., video but not audio). This logic keeps your original implementation.
      removeParticipant(consumer.appData.socketId);
    });
  }
